...
import com.example.campuslife.db.AppDatabase
...
class MainActivity : AppCompatActivity(),
    ...
    private fun loadDataFromDatabase() {
        lifecycleScope.launch {
            ...
            taskTabAdapter.setTasks(tasks.toMutableList())
            val notes = db.noteDao().getAll()
            notesAdapter.setNotes(notes.toMutableList())
    ...
    override fun onTaskLongClick(task: Task, position: Int) {
        AlertDialog.Builder(this).setTitle("Delete Task?").setPositiveButton("Yes") { _, _ ->
            val task = taskTabAdapter.getTask(position)
            lifecycleScope.launch {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            REQ_ADD_TASK -> {
                ...
                task?.let {
                    lifecycleScope.launch {
                        val newId = db.taskDao().insert(it).toInt()
                        val taskWithId = it.copy(id = newId)
                        taskTabAdapter.addTask(taskWithId)
                        showTab(rvTasksTab)
                        updatePlaceholderVisibility()
                    }
                }
            ...
            REQ_EDIT_TASK -> {
                ...
                task?.let {
                    if (editingPosition != -1) {
                        val originalId = taskTabAdapter.getTask(editingPosition).id
                        val taskWithId = it.copy(id = originalId)
                        lifecycleScope.launch {
                            db.taskDao().update(taskWithId)
                        }
                        taskTabAdapter.updateTask(taskWithId, editingPosition)
                    }
                    showTab(rvTasksTab)
                    updatePlaceholderVisibility()
                }
            ...
            REQ_NOTE -> {
                ...
                note?.let {
                    if (editingPosition != -1) {
                        val originalId = notesAdapter.getNote(editingPosition).id
                        val noteWithId = it.copy(id = originalId)
                        lifecycleScope.launch {
                            db.noteDao().update(noteWithId)
                        }
                        notesAdapter.updateNote(noteWithId, editingPosition)
                    } else {
                        lifecycleScope.launch {
                            val newId = db.noteDao().insert(it).toInt()
                            val noteWithId = it.copy(id = newId)
                            notesAdapter.addNote(noteWithId)
                            updatePlaceholderVisibility()
                        }
                    }
                    showTab(rvNotesTab)
                    if (editingPosition != -1) updatePlaceholderVisibility()
                }
            }
            ...
            REQ_EVENT -> {
                ...
                event?.let {
                    if (editingPosition != -1) {
                        val originalId = eventAdapter.getEvent(editingPosition).id
                        val eventWithId = it.copy(id = originalId)
                        lifecycleScope.launch {
                            db.eventDao().update(eventWithId)
                        }
                        eventAdapter.updateEvent(eventWithId, editingPosition)
                    } else {
                        lifecycleScope.launch {
                            val newId = db.eventDao().insert(it).toInt()
                            val eventWithId = it.copy(id = newId)
                            eventAdapter.addEvent(eventWithId)
                            updatePlaceholderVisibility()
                        }
                    }
                    showTab(rvEventsTab)
